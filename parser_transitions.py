...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        # If the transition is LEFT-ARC
        elif transition == "LA":
            # Pop the penultimate item off the stack and add it as a left dependent to the top item on the stack
            self.dependencies.append((self.stack[-1], self.stack.pop(-2)))
        # If the transition is RIGHT-ARC
        else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Repeat this until all of the sentences are parsed
    while len(unfinished_parses) != 0:
        # Get the next batch of transitions
        transitions = model.predict(unfinished_parses[:batch_size])
        # Parse the sentences in the batch using the predicted transitions
        [partial_parse.parse_step(transition) for partial_parse, transition in zip(unfinished_parses[:batch_size], transitions)]
        # Add any new dependencies to the list of dependencies
        dependencies += [(partial_parses.index(parsed), parsed.dependencies) for parsed in unfinished_parses[:batch_size] if len(parsed.buffer) == 0 and len(parsed.stack) == 1]
        # Remove any fully parsed sentences from the list of unfinished parses
